...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const monthlyTrends = await db.select({
      month: sql<string>`EXTRACT(MONTH FROM ${problemReports.createdAt})`,
      year: sql<string>`EXTRACT(YEAR FROM ${problemReports.createdAt})`,
      count: count()
    })
    .from(problemReports)
...
router.get('/admin-stats', requireRole(['admin']), async (req: Request, res: Response) => {
  try {
...
router.get('/agent-stats', requireRole(['admin', 'agent']), async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
